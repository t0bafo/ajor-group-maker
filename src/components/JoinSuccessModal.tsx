...
const JoinSuccessModal = ({ open, onOpenChange, groupName, onContinue, isPending = false }: JoinSuccessModalProps) => {
  return (
      ...
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogTitle className="text-center text-2xl">
            {isPending ? 'Request Submitted!' : `Welcome to ${groupName}!`}
          </DialogTitle>
        ...
        <div className="py-4 space-y-3">
          <div className={`rounded-lg p-4 border ${isPending ? 'bg-primary/10 border-primary/20' : 'bg-accent/10 border-accent/20'}`}>
            <p className="text-sm text-muted-foreground text-center">
              {isPending ? (
                <>
                  The host will review your request and notify you by email. 
                  You'll be able to access the group dashboard once approved.
                </>
              ) : (
                <>
        ...
        <DialogFooter>
          <Button variant="hero" onClick={onContinue} className="w-full" size="lg">
            {isPending ? 'Back to Dashboard' : 'Go to Group Dashboard'}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
